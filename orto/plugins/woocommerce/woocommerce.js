...
jQuery( document ).on( 'action.ready_orto', function() {
	...
	$document.on( 'action.resize_orto', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.orto_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		orto_set_cookie( 'orto_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// Add buttons inc/dec
	function orto_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						orto_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function orto_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_orto', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < orto_window_scroll_top() + orto_window_height() * 1.5 ) {
	...
	//-------------------------------------------------------------------------
	if ( ! ORTO_STORAGE['select_container_disabled'] ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		orto_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					orto_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					orto_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		orto_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			orto_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function orto_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = orto_window_width() + ( orto_window_height() < $document.height() || orto_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= ORTO_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
