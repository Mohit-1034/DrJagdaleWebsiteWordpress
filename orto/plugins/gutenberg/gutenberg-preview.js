...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			orto_create_observer( 'orto-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				orto_gutenberg_editor_init();
			} );
		} else {
		 	orto_gutenberg_editor_init();
		}
	...
	function orto_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'orto_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'orto_inited' ) ) {
	...
	// Init on page load
	function orto_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = orto_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + ORTO_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + ORTO_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in ORTO_STORAGE['override_classes'] ) {
				$obj.addClass( ORTO_STORAGE['override_classes'][i].replace( '%s', ORTO_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( orto_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && ORTO_STORAGE['sidebar_position'] != 'hide' ) {
				if ( orto_apply_filters( 'orto_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'orto_inited' );
		}

		// Widgets Editor
		//----------------------------------------------------------
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			if ( orto_apply_filters( 'orto_filter_add_scheme_to_widgets_editor', true ) ) {
				// Create an observer to add the class 'scheme_xxx' to the each new sidebar
				orto_remove_observer( 'orto-check-editor-styles-wrapper' );
				orto_create_observer( 'orto-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'orto_inited' );
	}
} );
