...
jQuery( document ).ready(
	function() {
		...
		orto_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="orto_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.orto_post_editor' );
						if ( link.length > 0 ) {
							orto_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.orto_post_editor', function(e) {
						orto_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('orto_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function orto_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
