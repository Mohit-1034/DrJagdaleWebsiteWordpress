...
if ( ! defined( 'ORTO_THEME_FREE' ) ) {
	define( 'ORTO_THEME_FREE', false );
}
if ( ! defined( 'ORTO_THEME_FREE_WP' ) ) {
	define( 'ORTO_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'ORTO_THEME_ALLOW_UPDATE' ) ) {
	define( 'ORTO_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'ORTO_ALLOW_SKINS' ) ) {
	define( 'ORTO_ALLOW_SKINS', true );
}
if ( ! defined( 'ORTO_DEFAULT_SKIN' ) ) {
	define( 'ORTO_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'ORTO_REMEMBER_SKIN' ) ) {
	define( 'ORTO_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'orto_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function orto_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = orto_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'orto_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'orto_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function orto_trx_updater_get_theme_info( $info ) {
		$engine = orto_storage_get( 'engine' );
...
if ( ! function_exists( 'orto_original_theme_slug' ) ) {
	add_filter( 'orto_filter_original_theme_slug', 'orto_original_theme_slug' );
	/**
...
if ( ! function_exists( 'orto_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'orto_importer_set_options', 9 );
	/**
	 ...
	 */
	function orto_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( orto_get_protocol() . ':' . orto_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( orto_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'orto_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'orto_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function orto_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( orto_get_protocol() . ':' . orto_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'orto_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'orto_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function orto_customizer_theme_setup1() {
		orto_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'orto_filter_allow_fullscreen' return false)
