<div class="front_page_section front_page_section_testimonials<?php
	$orto_scheme = orto_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $orto_scheme ) && ! orto_is_inherit( $orto_scheme ) ) {
		echo ' scheme_' . esc_attr( $orto_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( orto_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( orto_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$orto_css      = '';
		$orto_bg_image = orto_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$orto_anchor_icon = orto_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$orto_anchor_text = orto_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( orto_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$orto_css      = '';
			$orto_bg_mask  = orto_get_theme_option( 'front_page_testimonials_bg_mask' );
			$orto_bg_color_type = orto_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $orto_bg_color_type ) {
				$orto_bg_color = orto_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$orto_caption = orto_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $orto_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $orto_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $orto_caption, 'orto_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$orto_description = orto_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $orto_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $orto_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $orto_description ), 'orto_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! orto_exists_trx_addons() ) {
						...
						orto_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
