...
if ( function_exists( 'orto_elementor_is_preview' ) && orto_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			orto_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}
	...
	orto_new_main_query(
		array(
			'post_type'      => orto_get_theme_option( 'post_type' ),
			'category'       => orto_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'orto_filter_get_template_part', orto_blog_archive_get_template() ) );
}
