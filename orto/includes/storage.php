...
if ( ! function_exists( 'orto_storage_get' ) ) {
	/**
	 ...
	 */
	function orto_storage_get( $var_name, $default = '' ) {
		global $ORTO_STORAGE;
		return isset( $ORTO_STORAGE[ $var_name ] ) ? $ORTO_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'orto_storage_set' ) ) {
	/**
	 ...
	 */
	function orto_storage_set( $var_name, $value ) {
		global $ORTO_STORAGE;
		$ORTO_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'orto_storage_empty' ) ) {
	/**
	 ...
	 */
	function orto_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $ORTO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $ORTO_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $ORTO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'orto_storage_isset' ) ) {
	/**
	 ...
	 */
	function orto_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $ORTO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $ORTO_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $ORTO_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'orto_storage_unset' ) ) {
	/**
	 ...
	 */
	function orto_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $ORTO_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $ORTO_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $ORTO_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $ORTO_STORAGE[ $var_name ] ) ) {
				unset( $ORTO_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'orto_storage_inc' ) ) {
	/**
	 ...
	 */
	function orto_storage_inc( $var_name, $value = 1 ) {
		global $ORTO_STORAGE;
		if ( empty( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = 0;
		}
		$ORTO_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'orto_storage_concat' ) ) {
	/**
	 ...
	 */
	function orto_storage_concat( $var_name, $value ) {
		global $ORTO_STORAGE;
		if ( empty( $ORTO_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'orto_storage_get_array' ) ) {
	/**
	 ...
	 */
	function orto_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $ORTO_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $ORTO_STORAGE[ $var_name ][ $key ] ) ? $ORTO_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'orto_storage_set_array' ) ) {
	/**
	 ...
	 */
	function orto_storage_set_array( $var_name, $key, $value ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $ORTO_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$ORTO_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'orto_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function orto_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $ORTO_STORAGE[ $var_name ][ $key ] ) ) {
			$ORTO_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $ORTO_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$ORTO_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$ORTO_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'orto_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function orto_storage_merge_array( $var_name, $key, $value ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ORTO_STORAGE[ $var_name ] = array_merge( $ORTO_STORAGE[ $var_name ], $value );
		} else {
			$ORTO_STORAGE[ $var_name ][ $key ] = array_merge( $ORTO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'orto_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function orto_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			orto_array_insert_after( $ORTO_STORAGE[ $var_name ], $after, $key );
		} else {
			orto_array_insert_after( $ORTO_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'orto_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function orto_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			orto_array_insert_before( $ORTO_STORAGE[ $var_name ], $before, $key );
		} else {
			orto_array_insert_before( $ORTO_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'orto_storage_push_array' ) ) {
	/**
	 ...
	 */
	function orto_storage_push_array( $var_name, $key, $value ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $ORTO_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $ORTO_STORAGE[ $var_name ][ $key ] ) ) {
				$ORTO_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $ORTO_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'orto_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function orto_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $ORTO_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $ORTO_STORAGE[ $var_name ] ) && is_array( $ORTO_STORAGE[ $var_name ] ) && count( $ORTO_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $ORTO_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $ORTO_STORAGE[ $var_name ][ $key ] ) && is_array( $ORTO_STORAGE[ $var_name ][ $key ] ) && count( $ORTO_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $ORTO_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'orto_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function orto_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ORTO_STORAGE[ $var_name ][ $key ] ) ) {
			$ORTO_STORAGE[ $var_name ][ $key ] = 0;
		}
		$ORTO_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'orto_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function orto_storage_concat_array( $var_name, $key, $value ) {
		global $ORTO_STORAGE;
		if ( ! isset( $ORTO_STORAGE[ $var_name ] ) ) {
			$ORTO_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ORTO_STORAGE[ $var_name ][ $key ] ) ) {
			$ORTO_STORAGE[ $var_name ][ $key ] = '';
		}
		$ORTO_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'orto_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function orto_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $ORTO_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ORTO_STORAGE[ $var_name ] ) ? $ORTO_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ORTO_STORAGE[ $var_name ] ) ? $ORTO_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'orto_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function orto_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $ORTO_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $ORTO_STORAGE[ $var_name ]->$prop ) ? $ORTO_STORAGE[ $var_name ]->$prop : $default;
	}
}
